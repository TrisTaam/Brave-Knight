...
DSAttack::DSAttack(ICreep* creep) {
	m_creep = creep;
}

void DSAttack::Init() {
	m_animation = new Animation(*ASSET->getTexture("Creep/Demon/Demon_attack"), sf::Vector2i(8, 1), 0.1f);
	m_animation->setScale(m_creep->GetScale());
}

void DSAttack::Update(float deltaTime) {
...
void DSAttack::Render(sf::RenderWindow* window) {
